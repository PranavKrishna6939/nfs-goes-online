...
int main(int argc, char *argv[])
{
    ...
    while ((ch = getopt(argc, argv, "i:d:f:v")) != -1)
    {
        ...
        {
        case 'i':
            // for the number of inodes
            num_inodes = atoi(optarg);
            break;
        case 'd':
            // for the number of data blocks
            num_data = atoi(optarg);
            break;
        case 'f':
            // pass -f flag name of the filesystem image file
            image_file = optarg;
            break;
        case 'v':
            // pass -v flag for the visualization of layout of the filesystem image
            visual = 1;
            break;
        default:
            usage();
    ...
    // Allocate first inode in inode bitmap
    ...
    // Allocate first data block in data bitmap
    ...
    // Assumes 4096 block, 32 byte entries
